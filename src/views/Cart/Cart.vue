			...
			<table class="product-table">
				...
				<tbody>
					<span v-if="cartData.length === 0" style="color: #fff" class="no-product"
						>沒有商品，快去看看吧
						<router-link class="title" :to="{ path: '/Home' }">點我繼續購物</router-link>
					</span>
...
import Caculator from '../Product/ProductCaculator/Caculator.vue';
export default {
	name: '購物車表格',
	components: {
		Caculator,
	...
	computed: {
		...mapState(['cartData'].sort((a, b) => a.id - b.id)),
		...mapState(['checkData']),
	},
	methods: {
